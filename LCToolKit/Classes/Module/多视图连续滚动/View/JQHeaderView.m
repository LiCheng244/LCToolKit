...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if (tableViewoffsetY < 125){
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            self.searchBar.frame = CGRectMake(-(self.lc_width-60), 30, self.lc_width-80, 30);
        ...
        [UIView animateWithDuration:0.25 animations:^{
            self.searchBar.frame = CGRectMake(20, 30, self.lc_width-80, 30);
...
- (UISearchBar *)searchBar {
    ...
    if (!_searchBar) {
        _searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(-(self.lc_width-60), 30, self.lc_width-80, 30)];
        _searchBar.placeholder = @"搜索值得买的好物";
        _searchBar.layer.cornerRadius = 15;
        _searchBar.layer.masksToBounds = YES;

//        [_searchBar setSearchFieldBackgroundImage:[UIImage imageWithColor:[UIColor clearColor] size:_searchBar.lc_size] forState:UIControlStateNormal];
        
//        [_searchBar setBackgroundImage:[UIImage imageWithColor:[[UIColor grayColor] colorWithAlphaComponent:0.4] size:_searchBar.lc_size] ];
...
- (UIButton *)emailButton {
    if (!_emailButton) {
        _emailButton = [[UIButton alloc] initWithFrame:CGRectMake(self.lc_width-45, 30, 30, 30)];
